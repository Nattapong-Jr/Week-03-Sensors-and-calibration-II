...
| --- | -------------------------------------------------------------- | -------------------- | ------ |
| 1.1 | อธิบายหลักการทำงานของ LDR ได้ถูกต้อง                           |         5            |     เข้าใจพฤติกรรมที่ทำให้ความต้านทานเปลี่ยน   |
| 1.2 | อธิบายการทำงานภายใน LDR (โฟตอน → อิเล็กตรอน → conduction band) |            5          |    โฟตอนจากแสงทำให้ อิเล็กตรอนเคลื่อนที่ไปที่ conduction ทำให้ความต้านทานเปลี่ยน    | 
| 1.3 | เข้าใจว่าความสัมพันธ์ R vs Lux ไม่เป็นเส้นตรง                  |      3       |     ยังไม่คุ้นกับสมการ Power Low   |
| 1.4 | เข้าใจเหตุผลที่ต้องใช้ Voltage Divider                         |           5           |    microcontroll อ่านค่าความต้านทานตรงๆ ไม่ได้ต้องแปลงเป็นแรงดันก่อน V    |

### **ส่วนที่ 10.2 — การต่อวงจรและอ่านค่า ADC (20 คะแนน)**

| ข้อ | รายการ                              | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | ----------------------------------- | -------------------- | ------ |
| 2.1 | ต่อวงจร Voltage Divider ได้ถูกต้อง  |             5         |    ต่อสายจากขา GPI34 เข้ากับ LDR ได้ถูกต้อง    |
| 2.2 | เลือกค่า R คงที่เหมาะสม (เช่น 10kΩ) |                 3     |     ไม่ได้เลือกค่าโอมเองมี build มาให้ในบอร์ดแล้ว   |
| 2.3 | เขียนโปรแกรมอ่านค่า ADC ได้ถูกต้อง  |              5        |   สามารถเขียนโปรอกรมถาษา C ได้รัะดับหนึ่งเลยไม่มีปัญหา     |
| 2.4 | ใช้ ESP‑IDF ADC + Calibration ได้   |            4          |    อ่านค่า ADC ได้แต่ยังไม่ค่อยเข้าใจฟังก์ชัน Attenuation    |

### **ส่วนที่ 10.3 — การเก็บข้อมูลจริง (20 คะแนน)**

| ข้อ | รายการ                             | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | ---------------------------------- | -------------------- | ------ |
| 3.1 | เก็บข้อมูล ADC อย่างน้อย 5–7 จุด   |             5         |   เก็บข้อมูลมาทั้งหมด 8 จุด     |
| 3.2 | วัดค่า Lux จากมือถือได้ถูกต้อง     |            3          |    เพราะในแอปที่ใช้มีค่าความคลาดเคลื่อนอยู่บ้าง    |
| 3.3 | จดข้อมูลเป็นตารางครบถ้วน           |          5            |    แบ่งข้อมูลเป็นตารางครบถ้วน    |
| 3.4 | ควบคุมสภาพแสงให้คงที่ระหว่างการวัด |              3        |    เพราะมีแสงรบกวนเล็กน้อยจากหน้าต่างหรือส่วนอื่น    |
...
| ข้อ | รายการ                                        | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | --------------------------------------------- | -------------------- | ------ |
| 4.1 | สร้างกราฟ Scatter ADC vs Lux ได้              |           4           |    แสดงกราฟเส้นเปนียบเทียบไว้ในตารางพร้อม ข้อมูลที่เก้บมา    |
| 4.2 | เลือก Trendline เหมาะสม (Linear / Polynomial) |            3          |    ไม่รู้ว่าควรใช้ Polynomial ดีกรีเท่าไหร่ถึงจะพอดี    |
| 4.3 | อ่านสมการจากกราฟได้ถูกต้อง                    |                 4     |    ใช้โปรแกรม sheet สร้างกราฟมีเรื่องจุดทศนิยมที่ต้องระวัง    |
| 4.4 | นำสมการไปใช้ในโค้ดได้                         |                 4     |    ประยุกต์สมการทางคิดศาสตร์มาเขียนเป็นโปรแกรมภาษา C    |
| 4.5 | วิเคราะห์ว่า Linear หรือ Polynomial เหมาะกว่า |               3       |     เห็นจากค่า R-squared ว่า Polynomial แม่นยำกว่าแต่บอกไม่ได้ว่าทำไมแสงมากกับแสงน้อยถึงมีความชันต่างกันเยอะ   |
### **ส่วนที่ 10.5 — การทดสอบผลลัพธ์และวิเคราะห์ (15 คะแนน)**

| ข้อ | รายการ                                   | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | ---------------------------------------- | -------------------- | ------ |
| 5.1 | ทดสอบจุดแสงใหม่ที่ไม่ได้ใช้ตอนเก็บข้อมูล |                   5   |    วัดค่าแสงในสภาพแวดล้อมใหม่ที่ไม่ซ้ำกัตอน Calibration    |
| 5.2 | เปรียบเทียบ Lux มือถือ vs Lux จากโค้ด    |       5               |    รันโปรแกรมให้คำนวณค่า Lux มาเทียบกับมือถือ    |
| 5.3 | วิเคราะห์ความคลาดเคลื่อนและสาเหตุได้     |                  3    |    ไม่รู้ว่าความคลาดเคลื่อนมาจาก sensor ของโทรศัพท์หรือแสงรบกวนมากน้อยแค่ไหน    |
.................................................................
...
1. จุดไหนในใบงานที่คิดว่ายากที่สุด? 
    ...
    พฤติกรรมของ LDR แสงจะไปทำให้อิเล็กทรอนเคลื่อนที่ทำให้ความต้านทานเปลี่ยน
    
2. ถ้าทำใบงานนี้อีกครั้ง จะปรับปรุงอะไร? 

    สภาพแวดล้อมเพราะ ตอนในห้องเรียนมีปัญหาเรื่องแรงรบกวน
    
3. ส่วนไหนที่คิดว่าตนเองเข้าใจดีมาก? 

    แสงจะไปเปลี่ยนความต้านทานของ LDR แสงน้อยความต้านทานน้อยลง esp32 จะแปลงค่าความต้านทาน เป็น V เพื่ออ่านค่า เพราะ esp32 อ่านค่าความต้านทานโดยตรงไม่ได้
